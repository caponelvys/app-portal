...
export default function EditAppPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data) setForm({ name: data.name, description: data.description, url: data.url, icon: data.icon, status: data.status })
      setFetching(false)
    }
    load()
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('apps').update(form).eq('id', id)
  ...
  if (fetching) return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">Loading...</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-lg mx-auto px-6 py-10">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Edit App</h1>
