...
function deviceHostname(devices: Device[], deviceId: string) {
    ...
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-900 border border-gray-800 rounded-xl p-1 w-fit">
        <button className={tabClass('devices')} onClick={() => setTab('devices')}>
          Devices {devices.length > 0 && <span className="ml-1 text-xs text-gray-500">({devices.length})</span>}
        </button>
        <button className={tabClass('activity')} onClick={() => setTab('activity')}>
          Activity {logs.length > 0 && <span className="ml-1 text-xs text-gray-500">({logs.length})</span>}
        </button>
        <button className={tabClass('install')} onClick={() => setTab('install')}>
          Install Agent
        </button>
      </div>
              ...
              {devices.length > 0 ? devices.map((device) => (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-gray-500">
                    No devices enrolled yet. Go to Install Agent to get started.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <tbody>
              {logs.length > 0 ? logs.map((log) => (
                <tr key={log.id} className="border-b border-gray-800 hover:bg-gray-800">
                  ...
                  <td className="px-4 py-3 text-gray-400 text-xs">{deviceHostname(devices, log.device_id)}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{new Date(log.created_at).toLocaleString()}</td>
                </tr>
              )) : (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-gray-500">No activity yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {tab === 'install' && (
        <div className="space-y-6">
          <div>
            <p className="text-sm text-gray-500 mb-4">Download the installer and run it on any device you want to enroll.</p>
