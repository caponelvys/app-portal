...
SERVICE_FILE="/etc/systemd/system/ravyn-agent.service"
...
# Remove any prior "App Controller" service (pre-rebrand) so we don't run two
# agents. The old data dir is left in place so the agent can migrate its identity.
systemctl disable --now appcontroller 2>/dev/null || true
rm -f /etc/systemd/system/appcontroller.service
systemctl daemon-reload

echo "[install] Creating systemd service..."
cat > "$SERVICE_FILE" << EOF
[Unit]
Description=Ravyn Agent
...
Restart=always
RestartSec=5
StandardOutput=append:/var/log/ravyn-agent.log
StandardError=append:/var/log/ravyn-agent.log
...
systemctl daemon-reload
systemctl enable ravyn-agent
systemctl start ravyn-agent

echo "[install] Done! Agent is running."
echo "[install] Logs: journalctl -u ravyn-agent -f"
echo "[install] Or:   tail -f /var/log/ravyn-agent.log"
